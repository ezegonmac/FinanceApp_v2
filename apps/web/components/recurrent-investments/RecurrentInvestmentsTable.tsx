...
export default function RecurrentInvestmentsTable({
  recurrentInvestments,
  loading,
  error,
  onDelete,
  deletingId,
  ...
  if (!recurrentInvestments || recurrentInvestments.length === 0) {
    return <p>No recurrent investments configured.</p>;
  }
  ...
  const getStatusVariant = (status: RecurrentInvestment["status"]) => {
    if (status === "ACTIVE") return "success" as const;
    if (status === "PAUSED") return "warning" as const;
    return "destructive" as const;
  };

  const columns: ColumnDef<RecurrentInvestment>[] = [];
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="font-medium">
          {row.original.asset_ticker ?? row.original.asset_name ?? row.original.asset_id}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <Badge variant={row.original.type === "BUY" ? "success" : "destructive"}>
          {row.original.type}
        </Badge>
      ),
    },
