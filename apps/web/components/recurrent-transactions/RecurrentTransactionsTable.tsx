      ...
      <tbody>
        {recurrentTransactions.map((item) => (
          <tr key={item.id} style={{ borderBottom: "1px solid #ccc" }}>
            ...
            <td>
              <Link href={`/accounts/${item.to_account_id}`} style={{ color: "blue" }}>
                {item.to_account_name ?? item.to_account_id}
              </Link>
            </td>
            ...
            <td>{item.end_month ? new Date(item.end_month).toISOString().slice(0, 7) : "No end"}</td>
            <td>
