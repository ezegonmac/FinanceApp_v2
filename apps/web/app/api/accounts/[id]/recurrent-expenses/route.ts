...
export async function GET(request: Request, { params }: RouteContext) {
  try {
    ...
    return NextResponse.json(recurrentExpenses, { status: 200 });
  } catch (error) {
...
export async function POST(request: Request, { params }: RouteContext) {
  try {
    ...
    const parsed = recurrentExpenseSchema.parse(body);
    ...
    const recurrentExpense = await prisma.recurrentExpense.create({
      data: {
    ...
    if (recurrentExpense.status === "ACTIVE" && startMonthDate) {
      ...
      if (compareYearMonth(startYear, startMonth, endYear, endMonth) <= 0) {
        ...
        if (appliedCount > 0) {
          ...
          await prisma.recurrentExpense.update({
    ...
    return NextResponse.json(
      {
        error: "Failed to create recurrent expense",
        details: error instanceof Error ? error.message : "Unknown error",
      },
      { status: 500 }
    );
