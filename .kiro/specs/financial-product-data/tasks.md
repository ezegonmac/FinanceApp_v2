...
- [x] 3. Create shared TypeScript types and the `deriveGranularity` / `resolveTimeframeDates` functions
  ...
  - [ ]* 3.3 Write unit tests for `resolveTimeframeDates` and `deriveGranularity`
  ...
  - [ ]* 3.4 Write property test for `deriveGranularity` fallback (Property 4)
...
- [x] 4. Implement `computeMissingRanges` and `mergeSyncRange`
  ...
  - [ ]* 4.2 Write unit tests for `computeMissingRanges`
  ...
  - [ ]* 4.3 Write property test for `computeMissingRanges` (Property 2)
  ...
  - [ ]* 4.5 Write unit tests for `mergeSyncRange`
...
- [x] 5. Implement `syncPrices` and wire up the full sync algorithm
  ...
  - [ ]* 5.2 Write property test for sync range coverage (Property 1)
  ...
  - [ ]* 5.3 Write property test for sync idempotency (Property 3)
...
- [~] 6. Checkpoint — Ensure all algorithm tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Implement the Search API route
  - [~] 7.1 Create `apps/web/app/api/financial-products/search/route.ts`
  ...
  - [ ]* 7.2 Write unit tests for the Search API route
...
- [ ] 8. Implement the Assets API routes
  - [~] 8.1 Create `apps/web/app/api/financial-products/assets/route.ts`
    - Export `dynamic = "force-dynamic"`
  ...
  - [~] 8.2 Create `apps/web/app/api/financial-products/assets/[id]/route.ts`
    - Export `dynamic = "force-dynamic"`
  ...
  - [ ]* 8.3 Write unit tests for the Assets API routes
...
- [ ] 9. Implement the Prices API route
  - [~] 9.1 Create `apps/web/app/api/financial-products/prices/route.ts`
    - Export `dynamic = "force-dynamic"`
  ...
  - [ ]* 9.2 Write unit tests for the Prices API route
  ...
  - [ ]* 9.3 Write property test for price query window isolation (Property 5)
...
- [ ] 11. Implement the `/financial-products` page and `FinancialProductsView`
  - [~] 11.1 Create `apps/web/app/financial-products/page.tsx`
  ...
  - [~] 11.2 Create `apps/web/components/financial-products/FinancialProductsView.tsx`
    - Mark `'use client'`
...
- [ ] 12. Implement `TimeframeSelector`
  - [~] 12.1 Create `apps/web/components/financial-products/TimeframeSelector.tsx`
    - Mark `'use client'`
...
- [ ] 13. Implement `TrackedAssetList`
  - [~] 13.1 Create `apps/web/components/financial-products/TrackedAssetList.tsx`
    - Mark `'use client'`
...
- [ ] 14. Implement `AssetSearch`
  - [~] 14.1 Create `apps/web/components/financial-products/AssetSearch.tsx`
    - Mark `'use client'`
...
- [ ] 15. Implement `PriceChart`
  - [~] 15.1 Create `apps/web/components/financial-products/PriceChart.tsx`
    - Mark `'use client'`
...
- [ ] 16. Add navigation link and README TODO comment
  - [~] 16.1 Add a navigation link to `/financial-products` in the app's root layout or nav component (`apps/web/app/layout.tsx` or equivalent nav component)
    - Use a Lucide icon appropriate for financial products (e.g. `TrendingUp`)
    - Follow the existing nav link pattern in the layout

  - [~] 16.2 Add a TODO comment to the repo `README.md` describing the future cron job
...
- [ ] 17. Property test for tracked asset list consistency (Property 6)
  - [ ]* 17.1 Write property test for `GET /assets` consistency after add/delete operations (Property 6)
